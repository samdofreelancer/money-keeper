...
import { CreateBankAccountRequest, ValidationError } from './create-bank-account-flow.use-case';

export class ValidateAccountFormUseCase {
  execute(request: CreateBankAccountRequest): AccountFormValue {
    try {
      return new AccountFormValue(request as unknown as { [key: string]: unknown });
