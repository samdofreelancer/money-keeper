...
import { CategoryUseCasesFactory, CategoryCreationError, CategoryNotFoundError, CategoryValidationError } from '../CategoryUseCasesFactory';
...
describe('CategoryUseCasesFactory', () => {
  ...
  beforeEach(() => {
    ...
    factory = new CategoryUseCasesFactory(mockCategoryUiPort, mockCategoryApiPort);
  });

  describe('Constructor', () => {
    it('should accept injected dependencies', () => {
      const customFactory = new CategoryUseCasesFactory(mockCategoryUiPort, mockCategoryApiPort);
  ...
  describe('Input Validation', () => {
    ...
    it('should throw validation error for negative name length in generateUniqueName', () => {
      expect(() => factory.generateUniqueName(-1)).toThrow(CategoryValidationError);
    });
  });
  ...
  describe('ensureParentCategoryExists', () => {
    ...
    it('should return existing parent category ID when found', async () => {
      ...
      (mockCategoryApiPort.getAllCategories as Mock).mockResolvedValue([existingCategory]);
      ...
      expect(mockTrackingCallback).toHaveBeenCalledWith('existing-id', parentName, { isParent: true });
    });
    ...
    it('should create parent category when not found', async () => {
      ...
      (mockCategoryApiPort.createCategory as Mock).mockResolvedValue(createdCategory);
      ...
      expect(mockTrackingCallback).toHaveBeenCalledWith('new-id', parentName, { isParent: true });
    });
  ...
  describe('createCategory', () => {
    ...
    it('should handle category creation with tracking callback', async () => {
      ...
      expect(mockTrackingCallback).toHaveBeenCalledWith(categoryId, categoryName);
    });
  ...
  describe('createUniqueCategory', () => {
    ...
    it('should create unique category successfully', async () => {
      ...
      (mockCategoryUiPort.createUniqueCategory as Mock).mockResolvedValue(categoryId);

      const result = await factory.createUniqueCategory(categoryName, icon, type, {
        trackCreatedCategory: mockTrackingCallback,
      });
  ...
  describe('createChildCategory', () => {
    ...
    it('should create child category successfully via API', async () => {
      ...
      (mockCategoryApiPort.getAllCategories as Mock).mockResolvedValue([parentCategory]);
      (mockCategoryApiPort.createCategory as Mock).mockResolvedValue(childCategory);
      ...
      expect(mockTrackingCallback).toHaveBeenCalledWith('parent-id', parentName, { isParent: true });
      expect(mockTrackingCallback).toHaveBeenCalledWith('child-id', childName, { isParent: false });
  ...
  describe('createCategoryWithParentWorkflow', () => {
    it('should complete parent-child workflow successfully', async () => {
      ...
      (mockCategoryApiPort.getAllCategories as Mock).mockResolvedValue([parentCategory]);
      (mockCategoryUiPort.createUniqueCategory as Mock).mockResolvedValue('child-id');
  ...
  describe('Specialized Creation Methods', () => {
    it('should attempt to create category with exceeding max length', async () => {
      ...
      await factory.attemptToCreateCategoryWithNameExceedingMaxLength('icon', 'EXPENSE', onCreated);
    ...
    it('should create category with generated name', async () => {
      ...
      const result = await factory.createCategoryWithGeneratedName(15, 'icon', 'EXPENSE', onCreated);
    ...
    it('should attempt to create category with duplicate name', async () => {
      ...
      await factory.attemptToCreateCategoryWithDuplicateName('icon', 'EXPENSE', onCreated);
