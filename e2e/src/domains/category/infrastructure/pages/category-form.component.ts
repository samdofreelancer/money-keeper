...
export class CategoryFormComponent extends BasePage {
  ...
  async selectParentCategory(parent: string): Promise<void> {
  ...
  async submitCategoryExpectError(): Promise<void> {
    this.logger.info(`Submitting category expecting error`);
  ...
  async submitCategoryExpectSuccess(): Promise<string> {
    this.logger.info(`Submitting category expecting success`);
    const [response] = await Promise.all([
      this.page.waitForResponse(
        (resp) =>
          resp.url().includes("/categories") &&
    ...
    if (!response.ok()) {
      ...
      throw new Error(`Category submission failed with status ${status}`);
    }
    const data = await response.json();
    this.logger.info(`Category submitted successfully with id: ${data.id}`);
    return data.id;
  }
}
