...
Then(
  "I should see the search box with placeholder {string}",
...
Then(
  "I should see the table with columns:",
...
When(
  ...
  async function (this: CustomWorld, dataTable: { raw: () => string[][] }) {
    ...
    if (data.Name) {
      await this.page
        .locator('[data-testid="input-account-name"]')
        .fill(data.Name, { timeout: 5000 });
    }
    // Type (custom dropdown)
    if (data.Type) {
      // Click the select wrapper to open dropdown
      await this.page
        .locator('[data-testid="select-account-type"] .el-select__wrapper')
        .click({ timeout: 3000 });
    ...
    if (data.Balance) {
      await this.page
        .locator('[data-testid="input-account-balance"] input[type="number"]')
        .fill(data.Balance, { timeout: 5000 });
    }
  }
);
...
Given(
  "there is an account named {string}",
  async function (this: CustomWorld, name: string) {
    ...
    const exists = existing.data.some(
      (acc: Account) => acc.name === uniqueName
    );
    if (!exists) {
      await createAccount({
        name: uniqueName,
...
When(
  "I click the edit button for account {string}",
  async function (this: CustomWorld, name: string) {
    const accountsPage = new AccountsPage(this.page);
    const uniqueName = this.uniqueData?.get(name) || name;
    const row = accountsPage.page.getByRole("row", {
...
When(
  ...
  async function (this: CustomWorld, newName: string) {
    ...
    logger.info(
      `Unique data before update: ${
        this.uniqueData
          ? Array.from(this.uniqueData.entries())
              .map(([k, v]) => `${k} -> ${v}`)
              .join(", ")
          : "none"
      }`
    ...
    if (this.uniqueData) {
      for (const [, unique] of this.uniqueData.entries()) {
...
When(
  "I click the delete button for account {string}",
  async function (this: CustomWorld, name: string) {
    const accountsPage = new AccountsPage(this.page);
    const uniqueName = this.uniqueData?.get(name) || name;
    const row = accountsPage.page.getByRole("row", {
    ...
    if (await deleteBtn.count()) {
      ...
      await row
        .getByRole("button", { name: /Delete|Remove|delete/i })
        .click({ timeout: 5000 });
    }
  }
);
...
Given(
  ...
  async function (
    this: CustomWorld,
    dataTable: {
      hashes: () => { Name: string; Type: string; Balance: string }[];
    }
  ) {
    ...
    for (const { Name, Type, Balance } of dataTable.hashes()) {
      ...
      const uniqueName = `${Name}-${uuidv4().slice(0, 8)}`;
      if (!this.cleanupAccountNames)
        this.cleanupAccountNames = new Set<string>();
      this.cleanupAccountNames.add(uniqueName);
...
Then(
  "I should see {string} in the accounts table",
  async function (this: CustomWorld, name: string) {
    const accountsPage = new AccountsPage(this.page);
...
Then(
  "I should not see {string} in the accounts table",
  async function (this: CustomWorld, name: string) {
    const accountsPage = new AccountsPage(this.page);
    ...
    const visibleUnique = uniqueName
      ? await accountsPage.isAccountVisible(uniqueName)
      : false;
...
After(
  { tags: "@accounts and not @no-cleanup" },
