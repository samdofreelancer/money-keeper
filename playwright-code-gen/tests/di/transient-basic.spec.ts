...
test('Transient returns two different instances on two resolves', () => {
  ...
  (globalThis as unknown as { __DI_CONTAINER__?: Container }).__DI_CONTAINER__ = root;

  const scope = root.createScope();
  const a = scope.resolve<Holder>(Holder as unknown as new (...args: unknown[]) => Holder);
  const b = scope.resolve<Holder>(Holder as unknown as new (...args: unknown[]) => Holder);
