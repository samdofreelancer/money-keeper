...
export class Reporter {
  ...
  public static generateReport(): void {
    ...
    const passedTests = Reporter.testResults.filter(r => r.status === 'PASSED').length;
    const failedTests = Reporter.testResults.filter(r => r.status === 'FAILED').length;
    const skippedTests = Reporter.testResults.filter(r => r.status === 'SKIPPED').length;
    ...
    const reportPath = path.join(Reporter.reportDir, `report-${new Date().toISOString().replace(/:/g, '-')}.json`);
    fs.writeFileSync(reportPath, JSON.stringify(reportData, null, 2));
    
    Logger.info(`Report generated: ${reportPath}`);
    Logger.info(`Test Summary: ${passedTests}/${totalTests} passed (${failedTests} failed, ${skippedTests} skipped)`);
  }
}
