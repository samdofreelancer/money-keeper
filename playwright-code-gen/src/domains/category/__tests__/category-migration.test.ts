...
test.describe('Category Management Migration Test', () => {
  test('should create category without parent successfully', async ({
    page,
  }) => {
    ...
    const categoryExists = await categoryUseCase.categoryExists(
      categoryData.name
    );
    expect(categoryExists).toBe(true);

    // Delete the category
    await categoryUseCase.categoriesPage.clickDeleteCategoryButton(
      categoryData.name
    );
    await categoryUseCase.categoriesPage.clickConfirmDeleteButton();

    // Verify category is removed from the list
    const categoryNotExists = await categoryUseCase.categoryNotExists(
      categoryData.name
    );
