...
export class TransactionVerify {
  ...
  async shouldSeeTransaction(description: string) {
    ...
    const exists =
      await this.transactionsPage.verifyTransactionExists(description);
  ...
  async shouldHaveTransactionDetail(
    description: string,
    key: string,
    expectedValue: string
  ) {
    const actualValue = await this.transactionsPage.getTransactionDetail(
      description,
      key
    );
  ...
  async shouldHaveTransactionAmount(
    description: string,
    expectedAmount: string
  ) {
    const actualAmount = await this.transactionsPage.getTransactionDetail(
      description,
      'amount'
    );
