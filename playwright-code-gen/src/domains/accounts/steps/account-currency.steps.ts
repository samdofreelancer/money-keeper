...
Then('I should see the success message {string}', async function (message: string) {
  ...
  if (actualMessage && actualMessage.trim() !== message) {
    throw new Error(`Expected success message "${message}", but got "${actualMessage.trim()}"`);
  }
});

Then('the account {string} should appear in my accounts list with balance {string}', async function (accountName: string, balance: string) {
  const accountsPage = getAccountsPage();
  const isListed = await accountsPage.verifyAccountIsListed(accountName);
  if (!isListed) {
    throw new Error(`Account "${accountName}" not found in the accounts list`);
  }
  const actualBalance = await accountsPage.getAccountBalanceForRow(accountName);
  if (!actualBalance || actualBalance.trim() !== balance) {
    throw new Error(`Expected balance "${balance}", but got "${actualBalance?.trim()}"`);
  }
});

Then('the total balance should be correct for Euro currency', async function () {
  const accountsPage = getAccountsPage();
  const totalBalance = await accountsPage.getTotalBalance();
  if (!totalBalance.includes('€100.00')) {
    throw new Error(`Expected total balance to include "€100.00", but got "${totalBalance}"`);
  }
});
