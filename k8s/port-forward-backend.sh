#!/bin/bash
# Port Forwarding Script for Money Keeper Backend
# Platform: Linux/Mac (Bash)
# Usage: ./k8s/port-forward-backend.sh [-n namespace] [-p port] [-h]
...
if [ "$HELP" = true ]; then
    cat << EOF
${CYAN}Money Keeper - Backend Port Forwarding${NC}
Platform: Linux/Mac (Bash)

USAGE:
  ./k8s/port-forward-backend.sh [-n namespace] [-p port] [-h]
...
EXAMPLES:
  ...
  ./k8s/port-forward-backend.sh

  # Port forward to custom namespace
  ./k8s/port-forward-backend.sh -n custom-app

  # Use custom port
  ./k8s/port-forward-backend.sh -p 9000
...
echo -e "${CYAN}Money Keeper - Backend Port Forwarding${NC}"
...
if ! kubectl get svc -n "$NAMESPACE" money-keeper-backend-service &> /dev/null; then
    echo -e "${RED}ERROR: Backend service not found in namespace '$NAMESPACE'${NC}"
    exit 1
fi

echo -e "${GREEN}Backend service found - OK${NC}\n"
...
echo -e "  - Health Check: http://localhost:$PORT/actuator/health\n"

echo -e "${YELLOW}Port forwarding backend service...${NC}"
echo "Press Ctrl+C to stop port forwarding."

# Run port-forward in foreground
kubectl port-forward -n "$NAMESPACE" svc/money-keeper-backend-service "$PORT:8080"
